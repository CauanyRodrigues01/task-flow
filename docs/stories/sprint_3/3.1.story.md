...
- [x] **Backend: Serviço** (CA: 1, 3, 4)
  ...
  - [x] Implementar o método `createComment(Long taskId, CommentRequestDto commentDto, User author)` que:
    ...
    - [ ] (Nota: O registro no histórico de atividades será totalmente implementado na história 3.3).
...
- `Comment.java` created.
- `CommentRepository.java` created.
- `CommentRequestDto.java` created.
- `CommentResponseDto.java` created.
- `CommentService.java` created with `createComment` method and project membership validation.
- `CommentController.java` created with `POST /tasks/{taskId}/comments` endpoint and `@PreAuthorize`.
- `CommentServiceTest.java` (unit tests) created.
- `CommentControllerIntegrationTest.java` (integration tests) created.

### Completion Notes List
- Implementação completa do backend para a funcionalidade de comentários em tarefas, incluindo modelo, serviço, controlador e testes.
- A validação de permissão de comentários (membro do projeto) é realizada na camada de serviço.
- O registro no histórico de atividades será abordado na História 3.3, conforme especificado.
...
- `taskflow-backend/src/main/java/com.taskflow/comment/dto/CommentRequestDto.java`
- `taskflow-backend/src/main/java/com.taskflow.comment/dto/CommentResponseDto.java`
- `taskflow-backend/src/test/java/com/taskflow/comment/CommentServiceTest.java`
- `taskflow-backend/src/test/java/com/taskflow/comment/CommentControllerIntegrationTest.java`

## Log de Alterações
| Data | Versão | Descrição | Autor |
|---|---|---|---|
| 2025-11-06 | 1.0 | Rascunho inicial | sm |
| 2025-11-07 | 1.1 | Implementação do backend para comentários (modelo, serviço, controlador, testes) | James |
