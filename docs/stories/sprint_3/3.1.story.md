...
- [ ] **Backend: Modelo** (CA: 1, 2)
  - [ ] Criar a entidade JPA `Comment.java` no pacote `com.taskflow.comment` mapeando a tabela `comments`.
  - [ ] Criar a interface `CommentRepository.java` que estende `JpaRepository<Comment, Long>`.
  - [ ] Criar os DTOs `CommentRequestDto` e `CommentResponseDto`.
- [ ] **Backend: Serviço** (CA: 1, 3, 4)
  - [ ] Criar a classe `CommentService.java`.
  - [ ] Implementar o método `createComment(Long taskId, CommentRequestDto commentDto, User author)` que:
    - [ ] Valida a existência da tarefa (`Task`).
    - [ ] Verifica se o `author` tem permissão para comentar na tarefa (é membro do projeto).
...
- [ ] **Backend: Controlador** (CA: 1, 3)
  - [ ] Criar `CommentController.java` com o endpoint `POST /tasks/{taskId}/comments`.
  - [ ] Implementar a lógica do endpoint para receber a requisição, chamar o `CommentService` e retornar a resposta.
  - [ ] Proteger o endpoint com `@PreAuthorize` para garantir que apenas membros do projeto possam acessá-lo.
- [ ] **Backend: Testes** (CA: 5)
  - [ ] Escrever testes unitários para `CommentService` usando JUnit 5 e Mockito.
  - [ ] Escrever testes de integração para o `CommentController` usando `@SpringBootTest` e um banco de dados em memória (H2).
