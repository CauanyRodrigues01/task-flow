...
- [Fonte: architecture/8-especificao-da-api-restful.md (Modificação)]
...
## Tarefas / Subtarefas
- [ ] **Backend: DTO** (CA: 1, 2, 4)
  - [ ] Adicionar um campo opcional `String contextNote` ao DTO `TaskUpdateRequestDto.java`.
- [ ] **Backend: Serviço** (CA: 4)
  - [ ] Modificar o método de atualização de tarefa em `TaskService.java` para aceitar o `contextNote`.
  - [ ] Ao registrar a mudança de status no `ActivityHistoryService`, formatar a descrição para incluir a `contextNote` se ela estiver presente.
- [ ] **Backend: Testes** (CA: 6)
  - [ ] Atualizar os testes de integração para o endpoint de atualização de tarefa para incluir casos de teste com e sem a `contextNote`.
  - [ ] Verificar se o `ActivityHistory` é gravado corretamente com o contexto adicional.
