...
## Tarefas / Subtarefas
- [ ] **Backend: Modelo** (CA: 1, 2, 3, 4, 5)
  - [ ] Criar a entidade JPA `Notification.java` no pacote `com.taskflow.notification` mapeando a tabela `notifications`.
  - [ ] Criar a interface `NotificationRepository.java` que estende `JpaRepository<Notification, Long>`.
  - [ ] Criar o DTO `NotificationResponseDto`.
- [ ] **Backend: Serviço** (CA: 1, 2, 3, 4, 5)
  - [ ] Criar a classe `NotificationService.java` com métodos para:
    - [ ] `createNotification(Long userId, String message)`: Cria e salva uma notificação.
    - [ ] `getNotificationsByUserId(Long userId)`: Retorna as notificações de um usuário.
    - [ ] `markNotificationAsRead(Long notificationId)`: Marca uma notificação como lida.
  - [ ] Modificar `TaskService.java` para chamar `NotificationService` quando:
    - [ ] Uma tarefa é atribuída (CA: 1).
    - [ ] O status de uma tarefa é alterado (CA: 2).
  - [ ] Modificar `CommentService.java` para chamar `NotificationService` quando:
    - [ ] Um novo comentário é adicionado (CA: 3).
- [ ] **Backend: Controlador** (CA: 4, 5)
  - [ ] Criar `NotificationController.java` com os endpoints:
    - [ ] `GET /users/{userId}/notifications` para listar notificações.
    - [ ] `PATCH /notifications/{notificationId}/read` para marcar como lida.
  - [ ] Proteger os endpoints com `@PreAuthorize` para garantir as permissões corretas.
- [ ] **Backend: Testes** (CA: 6)
