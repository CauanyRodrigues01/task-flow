...
## Tarefas / Subtarefas
- [ ] **Backend: Modelo** (CA: 1, 2)
  - [ ] Criar a entidade JPA `ActivityHistory.java` no pacote `com.taskflow.activityhistory`.
  - [ ] Criar a interface `ActivityHistoryRepository.java`.
  - [ ] Criar o DTO `ActivityHistoryResponseDto`.
- [ ] **Backend: Serviço** (CA: 1, 2, 3)
  - [ ] Criar a classe `ActivityHistoryService.java` com um método `recordActivity(Long taskId, Long userId, String description)`.
  - [ ] Modificar `TaskService.java` para chamar `ActivityHistoryService` em todos os métodos que alteram dados da tarefa (criar, editar, atribuir, mudar status).
    - [ ] As descrições devem ser claras (ex: "Usuário 'X' alterou o status de 'A Fazer' para 'Em Progresso'").
    - [ ] Para mudanças de status, incluir o contexto (CA: 3).
  - [ ] Modificar `CommentService.java` para chamar `ActivityHistoryService` quando um novo comentário é adicionado.
- [ ] **Backend: Controlador** (CA: 4)
  - [ ] Criar `ActivityHistoryController.java` com o endpoint `GET /tasks/{taskId}/history`.
  - [ ] Implementar a lógica para buscar e retornar o histórico de uma tarefa.
  - [ ] Proteger o endpoint com `@PreAuthorize`.
- [ ] **Backend: Testes** (CA: 5)
  - [ ] Escrever testes unitários para `ActivityHistoryService`.
  - [ ] Escrever testes de integração para `ActivityHistoryController`.
  - [ ] Atualizar testes de `TaskService` e `CommentService` para verificar se o registro de histórico está sendo chamado.
